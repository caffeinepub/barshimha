    ...
    mutationFn: async ({ user, role }: { user: Principal, role: UserRole }) => {
      ...
      try {
        ...
        if (errorMessage.includes('Cannot change admin role to student')) {
          throw new Error('Admin role protection: Cannot change admin users to student role. Admin users must retain their administrative privileges.');
        } else if (errorMessage.includes('Unauthorized: Only admins can assign roles')) {
          throw new Error('You do not have permission to assign user roles. Only administrators can perform this action.');
        } else {
...
export function useUpdateUserActivity() {
  ...
  return useMutation({
    mutationFn: async ({ 
      ...
      if (!actor) throw new Error('Actor not available');
      return actor.updateUserActivity(loginCount, questionAttempts, commentCount, lastLogin);
    },
    onSuccess: () => {
...
export function useInitializeDomains() {
  ...
  return useMutation({
    mutationFn: async () => {
      if (!actor) throw new Error('Actor not available');
      return actor.initializeDomains();
    },
    onSuccess: () => {
...
// Domain and Topic Management Queries
export function useGetDomainTopics() {
  ...
  return useQuery<DomainTopic[]>({
    ...
    queryFn: async () => {
      if (!actor) return [];
      return actor.getDomainTopics();
    },
    enabled: !!actor && !isFetching,
  });
}

// New comprehensive queries for domain and topic management
export function useGetAllDomainsAndTopics() {
  ...
  return useQuery<DomainTopic[]>({
    ...
    queryFn: async () => {
      if (!actor) return [];
      return actor.getAllDomainsAndTopics();
    },
    enabled: !!actor && !isFetching,
  });
}
...
export function useAddDomainTopic() {
  ...
  return useMutation({
    mutationFn: async ({ id, name, type }: { id: string, name: string, type: DomainTopicType }) => {
      if (!actor) throw new Error('Actor not available');
      return actor.addDomainTopic(id, name, type);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['domains'] });
...
export function useUpdateDomainTopic() {
  ...
  return useMutation({
    mutationFn: async ({ id, newName }: { id: string, newName: string }) => {
      if (!actor) throw new Error('Actor not available');
      return actor.updateDomainTopic(id, newName);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['domains'] });
      queryClient.invalidateQueries({ queryKey: ['domainTopics'] });
...
export function useDeleteDomainTopic() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      if (!actor) throw new Error('Actor not available');
      return actor.deleteDomainTopic(id);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['domains'] });
      queryClient.invalidateQueries({ queryKey: ['domainTopics'] });
...
export function useGetQuestionsByFilter(domain?: string, difficulty?: string) {
  ...
  return useQuery<Question[]>({
    ...
    queryFn: async () => {
      if (!actor) return [];
      return actor.getQuestionsByFilter(
        domain || null,
        difficulty || null
      );
    },
...
export function useSaveCsvPreview() {
  ...
  return useMutation({
    mutationFn: async ({ previewId, preview }: { previewId: string, preview: CsvPreview }) => {
      if (!actor) throw new Error('Backend connection not available. Please check your authentication and try again.');
      ...
      try {
        ...
        if (errorMessage.includes('Unauthorized') || errorMessage.includes('admin')) {
          throw new Error('Admin access required to save CSV preview. Please ensure you have administrator privileges.');
        } else if (errorMessage.includes('Authentication required')) {
          throw new Error('Your session has expired. Please log in again to continue.');
        } else if (errorMessage.includes('CSV preview not found') || errorMessage.includes('expired')) {
          throw new Error('CSV preview has expired or was not found. Please re-upload your file and try again.');
        } else {
  ...
  return useQuery<CsvPreview | null>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (errorMessage.includes('Unauthorized') || errorMessage.includes('admin')) {
          throw new Error('Admin access required to view CSV preview. Please ensure you have administrator privileges.');
        } else if (errorMessage.includes('Authentication required')) {
          throw new Error('Your session has expired. Please log in again to continue.');
        } else if (errorMessage.includes('CSV preview not found') || errorMessage.includes('expired')) {
          throw new Error('CSV preview has expired or was not found. Please re-upload your file and try again.');
  ...
  return useMutation({
    mutationFn: async (previewId: string): Promise<CsvImportResult> => {
      ...
      try {
        ...
        if (errorMessage.includes('Unauthorized') || errorMessage.includes('admin')) {
          throw new Error('Admin access required to process CSV import. Please ensure you have administrator privileges.');
        } else if (errorMessage.includes('Authentication required')) {
          throw new Error('Your session has expired. Please log in again to continue.');
        } else if (errorMessage.includes('CSV preview not found') || errorMessage.includes('expired')) {
          throw new Error('CSV preview has expired or was not found. Please re-upload your file and try again.');
  ...
  return useMutation({
    mutationFn: async (previewId: string) => {
      ...
      try {
        ...
        } else if (errorMessage.includes('Unauthorized') || errorMessage.includes('admin')) {
...
export function useUploadPaymentProof() {
  ...
  return useMutation({
    mutationFn: async ({ paymentRecordId, filePath, fileType }: { paymentRecordId: string, filePath: string, fileType: string }) => {
      if (!actor) throw new Error('Backend connection not available. Please check your internet connection and try again.');
      ...
      try {
        ...
        if (errorMessage.includes('Authentication required')) {
          throw new Error('You must be logged in to upload payment proof. Please sign in and try again.');
        } else if (errorMessage.includes('Only students can upload payment proof')) {
          throw new Error('Only students are authorized to upload payment proof. Admin users cannot perform this action.');
        } else if (errorMessage.includes('You can only upload payment proof for your own records')) {
          throw new Error('You can only upload payment proof for your own payment records.');
        } else if (errorMessage.includes('Unauthorized')) {
          throw new Error('You are not authorized to perform this action. Please ensure you are logged in as a student.');
  ...
  return useMutation({
    mutationFn: async ({ 
      ...
      try {
        ...
        if (errorMessage.includes('Authentication required')) {
          throw new Error('You must be logged in to submit payment proof. Please sign in and try again.');
        } else if (errorMessage.includes('Only students can upload payment proof')) {
          throw new Error('Only students are authorized to submit payment proof. Admin users cannot perform this action.');
        } else if (errorMessage.includes('Unauthorized')) {
          throw new Error('You are not authorized to submit payment proof. Please ensure you are logged in as a student.');
        } else if (errorMessage.includes('network') || errorMessage.includes('timeout')) {
          throw new Error('Network error occurred during submission. Please check your internet connection and try again.');
        } else if (errorMessage.includes('file') || errorMessage.includes('storage')) {
          throw new Error('File upload failed. Please check your file and try again.');
        } else {
          throw new Error(`Submission failed: ${errorMessage}. Please try again or contact support if the problem persists.`);
        }
      }
    },
    onSuccess: async () => {
...
export function useUpdatePaymentConfiguration() {
  ...
  return useMutation({
    mutationFn: async ({ stcPayBarcodePath, localBankDetails }: { stcPayBarcodePath: string | null, localBankDetails: string | null }) => {
      if (!actor) throw new Error('Actor not available');
      return actor.updatePaymentConfiguration(stcPayBarcodePath, localBankDetails);
    },
    onSuccess: () => {
...
export function useUpdateBrandAssets() {
  ...
  return useMutation({
    mutationFn: async ({ logoPath, soundPath }: { logoPath: string | null, soundPath: string | null }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useSetAiAssistantConfig() {
  ...
  return useMutation({
    mutationFn: async ({ apiKey, enabled }: { apiKey: string, enabled: boolean }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useSendAiMessage() {
  ...
  return useMutation({
    mutationFn: async ({ messages }: { messages: Array<{ role: string; content: string }> }): Promise<string> => {
      if (!actor) throw new Error('Backend connection not available');
      ...
      try {
        ...
        if (errorMessage.includes('Payment required')) {
          throw new Error('Payment required: Please complete payment to access AI Assistant');
        } else if (errorMessage.includes('AI assistant is not configured')) {
          throw new Error('AI Assistant is not configured. Please contact your administrator.');
        } else if (errorMessage.includes('AI assistant is currently disabled')) {
          throw new Error('AI Assistant is currently disabled. Please contact your administrator.');
        } else if (errorMessage.includes('Unauthorized') || errorMessage.includes('Authentication required')) {
          throw new Error('Authentication required: Please log in to use AI Assistant');
