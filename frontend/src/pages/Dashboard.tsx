import { useGetCallerUserProfile, useGetQuestions, useGetComments, useGetCallerUserRole, useGetStudentProgress, useGetBookmarks, useGetStudyStatistics } from '../hooks/useQueries';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Progress } from '@/components/ui/progress';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { BookOpen, Brain, TrendingUp, Clock, CheckCircle, AlertCircle, Award, Flame, Bookmark, Target, BarChart3, Trophy, Play, Timer, BookmarkCheck } from 'lucide-react';
import { QuestionState, PaymentStatus, UserRole } from '../backend';
...
export function Dashboard() {
  ...
  useEffect(() => {
    if (userProfile?.paymentStatus === PaymentStatus.active && isStudent) {
      ...
      if (previousStatus && previousStatus !== PaymentStatus.active) {
        toast.success('🎉 Payment approved! You now have full access to all study materials and features.');
      }
  ...
  const stats = useMemo(() => {
    ...
    const domainStats = publishedQuestions.reduce((acc, question) => {
    ...
    const userComments = comments.filter(c => c.user.toString() === userProfile?.name);
  ...
  const progressData = useMemo(() => {
    ...
    const progressPercentage = stats.totalQuestions > 0 ? (completedQuestions / stats.totalQuestions) * 100 : 0;
  ...
  const realStudyStats = useMemo(() => {
    ...
    return {
      practiceQuestions: Number(studyStatistics.practiceModeStats.questionsAttempted),
      ...
      totalSessionsCompleted: Number(studyStatistics.aggregateStats.totalSessionsCompleted)
    };
  }, [studyStatistics]);
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <Badge className={getPaymentStatusColor(userProfile?.paymentStatus || PaymentStatus.pending)}>
            {getPaymentStatusText(userProfile?.paymentStatus || PaymentStatus.pending)}
          </Badge>
          {isStudent && (
            <Badge className={getPerformanceLevelColor(progressData.performanceLevel)}>
        ...
        <Card className="border-yellow-200 bg-yellow-50 dark:border-yellow-800 dark:bg-yellow-950">
          ...
          <CardContent>
            <CardDescription className="text-yellow-700 dark:text-yellow-300">
              Your payment is {userProfile?.paymentStatus === PaymentStatus.pending ? 'pending' : 'expired'}. 
              Please{' '}
              <Link 
                to="/profile" 
        ...
        <Card className="border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950">
          ...
          <CardContent>
            <CardDescription className="text-green-700 dark:text-green-300">
              Your subscription is active! You have full access to all study materials, questions, and features.
            </CardDescription>
          </CardContent>
        </Card>
      )}
      ...
      {isStudent && hasActivePayment && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{progressData.completedQuestions}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{progressData.accuracy.toFixed(1)}%</div>
              <p className="text-xs text-muted-foreground">
                {progressData.correctAnswers} of {progressData.totalAttempts} correct
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-orange-600">{progressData.streak}</div>
              <p className="text-xs text-muted-foreground">
                consecutive days
              </p>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{progressData.badges.length}</div>
              <p className="text-xs text-muted-foreground">
            ...
            <CardContent>
              {progressData.badges.length > 0 ? (
                ...
                <div className="text-center py-6">
                  ...
                  <p className="text-xs text-muted-foreground">Complete questions to unlock achievements!</p>
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent>
              {weakAreasChartData.length > 0 ? (
                ...
                <div className="text-center py-6">
                  ...
                  <p className="text-xs text-muted-foreground">Complete more questions to get personalized insights</p>
                </div>
              )}
            </CardContent>
          </Card>
      ...
      {isStudent && hasActivePayment && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Real-time progress across different study modes with dynamic updates
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Unlimited practice sessions</p>
                  </div>
                </div>
                  ...
                  {studyStatistics && (
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span>Accuracy:</span>
                        <span className="font-medium">{realStudyStats.practiceAccuracy.toFixed(1)}%</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>Correct:</span>
                        <span className="font-medium">{Number(studyStatistics.practiceModeStats.questionsCorrect)}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>Time Spent:</span>
                        <span className="font-medium">{formatTime(Number(studyStatistics.practiceModeStats.timeSpent))}</span>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Exam simulation sessions</p>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="text-center space-y-1">
                  ...
                  {studyStatistics && (
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span>Accuracy:</span>
                        <span className="font-medium">{realStudyStats.timedAccuracy.toFixed(1)}%</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>Questions:</span>
                        <span className="font-medium">{Number(studyStatistics.timedModeStats.questionsAttempted)}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>Time Spent:</span>
                        <span className="font-medium">{formatTime(Number(studyStatistics.timedModeStats.timeSpent))}</span>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Question review sessions</p>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="text-center space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">Review Sessions</p>
                  </div>
                  {studyStatistics && (
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span>Accuracy:</span>
                        <span className="font-medium">{realStudyStats.reviewAccuracy.toFixed(1)}%</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>Questions:</span>
                        <span className="font-medium">{Number(studyStatistics.reviewModeStats.questionsAttempted)}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>Time Spent:</span>
                        <span className="font-medium">{formatTime(Number(studyStatistics.reviewModeStats.timeSpent))}</span>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="mt-6 pt-6 border-t">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center">
                    <div className="text-lg font-bold">{Number(studyStatistics.aggregateStats.totalQuestionsAttempted)}</div>
                    <p className="text-xs text-muted-foreground">Total Questions</p>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold">{studyStatistics.aggregateStats.overallAccuracy.toFixed(1)}%</div>
                    <p className="text-xs text-muted-foreground">Overall Accuracy</p>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold">{realStudyStats.totalSessionsCompleted}</div>
                    <p className="text-xs text-muted-foreground">Total Sessions</p>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold">{formatTime(realStudyStats.totalTimeSpent)}</div>
            ...
            {!studyStatistics && (
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground mb-2">No study statistics yet</p>
                <p className="text-sm text-muted-foreground">
                  Start practicing questions to see your progress across different study modes
                </p>
              </div>
            )}
          </CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Link to="/study">
            <Card className="cursor-pointer hover:shadow-md transition-shadow">
              ...
              <CardContent>
                <CardDescription>
                  Start practicing with unlimited questions and immediate feedback
                </CardDescription>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-center py-4">
              <p className="text-sm text-muted-foreground">
                You have {bookmarks.length} bookmarked question{bookmarks.length !== 1 ? 's' : ''} ready for review
              </p>
            </div>
          </CardContent>
        </Card>
