...
import { useGetCsvPreview, useDeleteCsvPreview, useProcessCsvPreview, useCancelCsvPreview, useSaveCsvPreview, useIsCallerAdmin } from '../hooks/useQueries';
...
import type { CsvPreview as CsvPreviewType, ValidationResult } from '../backend';
import {
  ContextMenu,
  ContextMenuContent,
  ContextMenuItem,
...
export function CsvPreview() {
  ...
  const { data: previewData, isLoading, error, refetch } = useGetCsvPreview(previewId);
  const deleteCsvPreview = useDeleteCsvPreview();
  ...
  const reparseWithDelimiter = useCallback((content: string, delimiter: string) => {
    try {
  ...
  const handleDelimiterChange = async (newDelimiter: string) => {
    if (!rawFileContent) {
      toast.error('Cannot change delimiter: original file content not available');
      return;
    }
    ...
    if (reparsed) {
      ...
      const delimiterName = newDelimiter === '\t' ? 'tab' : newDelimiter === ';' ? 'semicolon' : 'comma';
      toast.success(`Data re-parsed with ${delimiterName} delimiter. ${reparsed.rows.length} rows detected.`);
    }
  };
  ...
  const handleAuthError = () => {
    if (!isAuthenticated) {
      ...
      toast.error('Admin access required. Please ensure you have the necessary permissions.');
      navigate({ to: '/admin' });
    }
  };
  ...
  const handlePreviewError = (error: any) => {
    ...
    if (errorMessage.includes('CSV preview not found') || errorMessage.includes('expired')) {
      toast.error('CSV preview has expired or was not found. Please re-upload your file.');
      navigate({ to: '/admin' });
    } else if (errorMessage.includes('Unauthorized') || errorMessage.includes('admin')) {
      toast.error('You do not have permission to access this preview. Admin access required.');
      navigate({ to: '/admin' });
  ...
  const validateRow = (rowIndex: number, rowData: string[], headers: string[] = editedHeaders, allRows: string[][] = editedData) => {
    const errors: string[] = [];
    let isValid = true;

    // Check for empty required fields
    const requiredFields = ['id', 'content', 'options', 'explanation', 'domain'];
    ...
    if (rowData.some(cell => cell.toLowerCase().includes('undefined') || cell.toLowerCase().includes('null'))) {
  ...
  const deleteSelectedRows = () => {
    ...
    rowsToDelete.forEach(rowIndex => {
      ...
      newValidationResults = newValidationResults.filter((_, idx) => idx !== rowIndex);
    });
  ...
  const addMultipleRows = (count: number) => {
    const newRows = Array.from({ length: count }, () => new Array(editedHeaders.length).fill(''));
  ...
  const performFindReplace = (replaceAll: boolean) => {
    ...
    let replacedCount = 0;
    const newData = editedData.map((row, rowIndex) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.errors.length > 0) {
          ...
          toast.error(`Errors: ${errorSummary}${result.errors.length > 3 ? '...' : ''}`);
        }
      }
    } catch (error: any) {
      handlePreviewError(error);
  ...
  const handleCancelImport = async () => {
    try {
      ...
      toast.info(`${fileName.endsWith('.tsv') ? 'TSV' : 'CSV'} import cancelled`);
    } catch (error: any) {
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center justify-center py-16">
            <div className="text-center space-y-4">
              ...
              <p className="text-lg text-muted-foreground">Verifying permissions and loading preview...</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-2">Your session has expired. Please log in again to continue.</p>
            </div>
          ...
          <div className="text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-2">
                You do not have administrator privileges required to access the CSV preview feature.
              </p>
            </div>
            <Button variant="outline" onClick={() => navigate({ to: '/admin' })}>
              <ArrowLeft className="h-4 w-4 mr-2" />
  ...
  // Show loading state for preview data
  if (isLoading) {
    return (
      <div className="min-h-screen bg-background">
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center justify-center py-16">
            <div className="text-center space-y-4">
              <div className="animate-spin rounded-full h-12 w-12 border-4 border-primary border-t-transparent mx-auto"></div>
              <p className="text-lg text-muted-foreground">Loading CSV/TSV preview...</p>
              <p className="text-sm text-muted-foreground">Preview ID: {previewId}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-2">
                {errorMessage.includes('expired') || errorMessage.includes('not found')
                  ? 'The CSV preview has expired or was not found. Please re-upload your file.'
                  : `Error: ${errorMessage}`
                }
              </p>
            </div>
  ...
  const delimiterName = currentDelimiter === '\t' ? 'Tab' : currentDelimiter === ';' ? 'Semicolon' : 'Comma';
        ...
        <div className="container-fluid mx-auto px-2 py-4 space-y-4 max-w-[98vw]">
          ...
          <div className="bg-gradient-to-r from-primary/5 to-secondary/5 rounded-lg p-4 border">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold">{fileType} Import Preview</h1>
              ...
              <div className="flex items-center space-x-2">
                <Label htmlFor="delimiter-select" className="text-sm font-medium">
                  Delimiter:
                </Label>
                <Select value={currentDelimiter} onValueChange={handleDelimiterChange}>
            ...
            <div className="flex items-start space-x-2 text-sm">
              ...
              <div>
                <strong className="text-foreground">{fileName}</strong> - Review and edit your {fileType} content. 
                Change the delimiter above if auto-detection was incorrect. Data will be re-parsed automatically.
                Use <kbd className="px-1 py-0.5 bg-muted rounded text-xs">Ctrl+Click</kbd> for multi-select, <kbd className="px-1 py-0.5 bg-muted rounded text-xs">Shift+Click</kbd> for range select.
              </div>
            </div>
          </div>
              ...
              <CardContent className="p-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-purple-600">{editedHeaders.length}</p>
                  </div>
              ...
              <CardContent className="p-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-orange-600">{hasUnsavedChanges ? '1' : '0'}</p>
                  </div>
          ...
          <div className="flex items-center justify-between bg-muted/20 p-3 rounded-lg border flex-wrap gap-2">
            <div className="flex items-center space-x-2 flex-wrap gap-2">
              <Button
                ...
                disabled={processCsvPreview.isPending || cancelCsvPreview.isPending}
              >
                ...
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center space-x-3">
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor="caseSensitive" className="cursor-pointer text-sm">
                        Case sensitive
                      </Label>
                    </div>
                    {matchCount > 0 && (
                    ...
                    <Table>
                      <TableHeader className="sticky top-0 bg-background z-20 shadow-sm">
                        <TableRow className="border-b-2">
                            ...
                            <TableHead className="w-12 text-center bg-muted/50 sticky left-0 z-10">
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <button onClick={toggleAllRows} className="p-1">
                          ...
                          <TableHead className="w-16 text-center font-bold bg-muted/50 sticky left-0 z-10">#</TableHead>
                          {editedHeaders.map((header, index) => (
                            <TableHead 
                              key={index} 
                              className="bg-muted/50"
                              style={{ minWidth: `${columnWidths[index] || 200}px` }}
                            >
                              <div className="space-y-1">
                                {isEditing && (
                                  <div className="flex items-center justify-center mb-1">
                                    <Tooltip>
                                      ...
                                      <TooltipContent>
                                        {selectedColumns.has(index) ? 'Deselect column' : 'Select column'}
                                      </TooltipContent>
                                    </Tooltip>
                                  </div>
                                )}
                                {isEditing ? (
                                  ...
                                  <div className="font-bold text-sm py-1 px-1">{header}</div>
                                )}
                              </div>
                            </TableHead>
                          ))}
                        ...
                        {visibleRows.map((row, visibleIndex) => {
                          ...
                          return (
                            <ContextMenu key={rowIndex}>
                                    ...
                                    return (
                                      ...
                                      >
                                        {isEditing && isEditingThisCell ? (
                                          ...
                                          >
                                            <span className="truncate">{cell || <span className="text-muted-foreground italic text-xs">Empty</span>}</span>
                                          </div>
                                        )}
                                      </TableCell>
                                    );
                              ...
                              <ContextMenuContent>
                                <ContextMenuItem onClick={() => toggleRowSelection(rowIndex)}>
          ...
          <div className="bg-muted/20 p-4 rounded-lg border">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center space-x-3">
                <Badge 
                  variant={invalidRows === 0 && !hasUnsavedChanges ? "default" : "destructive"}
                  className="text-sm"
                >
                  {invalidRows === 0 && !hasUnsavedChanges ? 'Ready to Import' : 
                   hasUnsavedChanges ? 'Unsaved Changes' : `${invalidRows} Errors`}
                </Badge>
                <div className="text-sm text-muted-foreground">
                  <strong>{editedData.length}</strong> rows • <strong>{editedHeaders.length}</strong> columns • <strong>{delimiterName}</strong> delimiter
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <Button
                  ...
                  disabled={invalidRows > 0 || hasUnsavedChanges || processCsvPreview.isPending || cancelCsvPreview.isPending || editedData.length === 0}
                  size="sm"
                >
