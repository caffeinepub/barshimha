...
import { useGetPaymentRecords, useAddPaymentRecord, useUpdatePaymentRecord, useApprovePayment } from '../../hooks/useQueries';
...
import { Search, Plus, Edit, Eye, FileText, Image, RefreshCw, CheckCircle, Clock, AlertCircle, Check } from 'lucide-react';
import { PaymentRecord, PaymentStatus, PaymentMethod, PaymentProof } from '../../backend';
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        Uploaded: {new Date(Number(proof.uploadedAt) / 1000000).toLocaleString()}
      </p>
    </div>
  );
}
...
export function AdminPayments() {
  const { data: paymentRecords = [], refetch: refetchPaymentRecords, isLoading: isLoadingRecords } = useGetPaymentRecords();
  ...
  const filteredPayments = paymentRecords.filter(payment => {
    const matchesSearch = payment.user.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleApprovePayment = async (paymentRecordId: string) => {
    try {
      ...
      toast.success('Payment approved successfully! Student payment status has been updated to active and they now have full access to all features.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const paymentData: PaymentRecord = {
        id: editingPayment?.id || `payment_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        user: principal,
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage student payments and subscriptions with real-time approval system</p>
        </div>
        <div className="flex items-center space-x-2">
          <Button
            variant="outline"
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  {editingPayment ? 'Update the payment details below.' : 'Fill in the payment details.'}
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, userPrincipal: e.target.value }))}
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value={PaymentMethod.stcBank}>STC Bank</SelectItem>
                      <SelectItem value={PaymentMethod.localBank}>Local Bank</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="flex justify-end space-x-2">
                  ...
                  <Button 
                    type="submit" 
                    disabled={addPaymentRecord.isPending || updatePaymentRecord.isPending}
                  >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">From active payments</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{activePayments.length}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{recordsWithProofs.length}</div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <p className="text-sm text-blue-800 dark:text-blue-200">
                Real-time synchronization active - Payment approvals immediately update student access across all interfaces
              </p>
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            All payment records with real-time approval system - approving payments immediately grants student access
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoadingRecords ? (
            <div className="flex items-center justify-center py-8">
              ...
              <p className="ml-2 text-muted-foreground">Loading payment records...</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                  ...
                  <TableRow key={payment.id}>
                    ...
                    <TableCell>
                      <div className="text-sm text-muted-foreground">
                        {new Date(Number(payment.createdAt) / 1000000).toLocaleDateString()}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="text-sm text-muted-foreground">
                        {new Date(Number(payment.updatedAt) / 1000000).toLocaleDateString()}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        {payment.status === PaymentStatus.pending && payment.proof && (
                          <Button
                            size="sm"
                            variant="default"
