...
import { useGetBrandAssets, useUpdateBrandAssets } from '../../hooks/useQueries';
...
import { Upload, Image, Music, Settings, Save, Eye, CheckCircle, AlertCircle, Loader2, RefreshCw, Trash2 } from 'lucide-react';
...
export function AdminBrandManagement() {
  ...
  const { data: currentLogoUrl, refetch: refetchLogoUrl } = useFileUrl(brandAssets?.logoPath || '');
  const { data: currentSoundUrl, refetch: refetchSoundUrl } = useFileUrl(brandAssets?.soundPath || '');
  ...
  const handleSoundFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
  ...
  const handleSaveBrandAssets = async () => {
    ...
    try {
      ...
      if (selectedLogoFile) {
        ...
        try {
          const uploadResult = await uploadFile(filePath, selectedLogoFile, (progress) => {
          ...
          throw new Error(`Logo upload failed: ${uploadError.message || 'Unknown error'}`);
        }
      }
      ...
      if (selectedSoundFile) {
        ...
        try {
          const uploadResult = await uploadFile(filePath, selectedSoundFile, (progress) => {
          ...
          throw new Error(`Sound upload failed: ${uploadError.message || 'Unknown error'}`);
        }
      }
      ...
      toast.success('Brand assets updated and deployed successfully! Changes are now live across the entire system.');
    } catch (error: any) {
  ...
  const cacheBustedLogoUrl = currentLogoUrl ? `${currentLogoUrl}?t=${Date.now()}` : null;
  const cacheBustedSoundUrl = currentSoundUrl ? `${currentSoundUrl}?t=${Date.now()}` : null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Upload and manage your platform's logo and landing page sound file with instant deployment
          </p>
        </div>
        <Button
          variant="outline"
                  ...
                  {brandAssets.logoPath ? (
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-green-600">Configured and active</span>
                      </div>
                      {cacheBustedLogoUrl && (
                        <div className="border rounded-lg p-3 bg-muted/30">
                          <img
                            ...
                            onError={() => toast.error('Failed to load current logo')}
                          />
                        </div>
                      )}
                    </div>
      ...
      <Alert className="border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950">
        ...
        <AlertDescription className="text-blue-800 dark:text-blue-200">
          <strong>Instant Deployment:</strong> Brand asset changes are deployed immediately across all pages and interfaces. 
          The logo will appear system-wide instantly, and the sound file will be immediately available on the landing page with enhanced reliability.
        </AlertDescription>
      </Alert>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload a logo image that will be displayed across all pages and interfaces with instant deployment
          </CardDescription>
        </CardHeader>
          ...
          <div className="space-y-3">
            ...
            <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-6 text-center hover:border-primary/50 transition-colors">
              <input
                ...
                disabled={isProcessing}
              />
              <label htmlFor="logo-upload" className={`cursor-pointer ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`}>
                <div className="space-y-3">
                  {selectedLogoFile ? (
                    <div className="space-y-3">
                      <div className="flex items-center justify-center space-x-2">
                        ...
                        <div className="text-center">
                          <span className="text-sm font-medium block">{selectedLogoFile.name}</span>
                          <span className="text-xs text-muted-foreground">{formatFileSize(selectedLogoFile.size)}</span>
                        </div>
                      </div>
                      {logoPreviewUrl && (
                        <div className="space-y-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload a sound file that will be available on the landing page with enhanced audio controls and reliability
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {/* Sound File Upload */}
          <div className="space-y-3">
            ...
            <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-6 text-center hover:border-primary/50 transition-colors">
              <input
                ...
                disabled={isProcessing}
              />
              <label htmlFor="sound-upload" className={`cursor-pointer ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`}>
                <div className="space-y-3">
                  {selectedSoundFile ? (
                    <div className="space-y-3">
                      <div className="flex items-center justify-center space-x-2">
                        ...
                        <div className="text-center">
                          <span className="text-sm font-medium block">{selectedSoundFile.name}</span>
                          <span className="text-xs text-muted-foreground">{formatFileSize(selectedSoundFile.size)}</span>
                        </div>
                      </div>
                      <Button
                        type="button"
        ...
        <CardContent className="pt-6">
          <div className="space-y-3">
            ...
            <ul className="text-sm text-green-700 dark:text-green-300 space-y-2">
              <li>• <strong>Logo:</strong> Clear and readable at different sizes, supports PNG (with transparency), SVG (scalable), JPG, and WebP formats</li>
              <li>• <strong>Sound:</strong> High-quality audio with enhanced player controls including play/pause, volume, and progress tracking</li>
              <li>• <strong>Instant Deployment:</strong> Changes are deployed immediately across all system interfaces with real-time updates</li>
              <li>• <strong>Reliability:</strong> Enhanced error handling and fallback mechanisms ensure consistent user experience</li>
              <li>• <strong>File Validation:</strong> Comprehensive validation ensures only compatible, high-quality files are uploaded</li>
              <li>• <strong>Progress Tracking:</strong> Real-time upload progress and status updates keep you informed throughout the process</li>
            </ul>
          </div>
        </CardContent>
      </Card>
