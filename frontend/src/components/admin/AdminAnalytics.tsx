...
import { useGetQuestions, useGetComments, useGetPaymentRecords, useGetUserStats, useGetUserActivities } from '../../hooks/useQueries';
...
import { BookOpen, MessageCircle, DollarSign, Users, TrendingUp, Activity, Calendar, UserCheck } from 'lucide-react';
import { QuestionState, PaymentStatus } from '../../backend';
...
export function AdminAnalytics() {
  ...
  const analytics = useMemo(() => {
    ...
    const draftQuestions = questions.filter(q => q.state === QuestionState.draft);
    const archivedQuestions = questions.filter(q => q.state === QuestionState.archived);

    // Domain distribution
    const domainData = publishedQuestions.reduce((acc, question) => {
    ...
    // Payment analytics
    const activePayments = paymentRecords.filter(p => p.status === PaymentStatus.active);
    const pendingPayments = paymentRecords.filter(p => p.status === PaymentStatus.pending);
    const expiredPayments = paymentRecords.filter(p => p.status === PaymentStatus.expired);

    const totalRevenue = activePayments.reduce((sum, payment) => sum + Number(payment.amount), 0);
    ...
    const totalLogins = userActivities.reduce((sum, activity) => sum + Number(activity.loginCount), 0);
    const totalQuestionAttempts = userActivities.reduce((sum, activity) => sum + Number(activity.questionAttempts), 0);
    const totalCommentCount = userActivities.reduce((sum, activity) => sum + Number(activity.commentCount), 0);

    // User engagement levels
    const highEngagementUsers = userActivities.filter(a => Number(a.loginCount) >= 10).length;
    const mediumEngagementUsers = userActivities.filter(a => Number(a.loginCount) >= 3 && Number(a.loginCount) < 10).length;
    const lowEngagementUsers = userActivities.filter(a => Number(a.loginCount) < 3).length;

    const engagementData = [
      { name: 'High (10+ logins)', value: highEngagementUsers, color: '#22c55e' },
      { name: 'Medium (3-9 logins)', value: mediumEngagementUsers, color: '#f59e0b' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium">{analytics.publishedQuestions}</span>
            </div>
            <div className="flex justify-between">
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            <div className="flex justify-between">
              <span className="text-sm text-muted-foreground">Question Attempts:</span>
              <span className="font-medium">{analytics.totalQuestionAttempts}</span>
            </div>
            <div className="flex justify-between">
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-green-600">{analytics.activePayments}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-muted-foreground">Pending:</span>
              <span className="font-medium text-yellow-600">{analytics.pendingPayments}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-muted-foreground">Expired:</span>
              <span className="font-medium text-red-600">{analytics.expiredPayments}</span>
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              ...
              <div className="text-sm text-muted-foreground">High Engagement Users</div>
              <div className="text-xs text-muted-foreground mt-1">10+ logins</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-yellow-600 mb-2">
                {analytics.mediumEngagementUsers}
              </div>
              <div className="text-sm text-muted-foreground">Medium Engagement Users</div>
              <div className="text-xs text-muted-foreground mt-1">3-9 logins</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-red-600 mb-2">
                {analytics.lowEngagementUsers}
              </div>
              <div className="text-sm text-muted-foreground">Low Engagement Users</div>
              <div className="text-xs text-muted-foreground mt-1">&lt;3 logins</div>
            </div>
          </div>
        </CardContent>
      </Card>
