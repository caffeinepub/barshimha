...
import { AlertCircle, Bot, CheckCircle, Eye, EyeOff, Save } from "lucide-react";
import { useEffect, useState } from "react";
import { toast } from "sonner";
import {
  useDeleteAiAssistantConfig,
  useGetAiAssistantConfig,
  useSetAiAssistantConfig,
} from "../../hooks/useQueries";

export function AdminAiConfig() {
  const { data: config, isLoading: _isLoading } = useGetAiAssistantConfig();
  ...
  const handleDelete = async () => {
    if (
      !confirm(
        "Are you sure you want to delete the AI Assistant configuration? This will disable the feature for all users.",
      )
    ) {
      return;
    }
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <CardDescription>
                Enter your DeepSeek API credentials to enable AI-powered study
                assistance
              </CardDescription>
            </div>
          </div>
        ...
        <CardContent className="space-y-6">
          <Alert>
            ...
            <AlertDescription>
              The AI Assistant uses DeepSeek's API to provide intelligent
              responses to student queries. You need a valid API key from
              DeepSeek to enable this feature.
            </AlertDescription>
          </Alert>
            ...
            <div className="space-y-1">
              ...
              {config ? (
                <div className="flex items-center space-x-2 text-sm text-green-600">
                  ...
                  <span>
                    Configured and {config.enabled ? "enabled" : "disabled"}
                  </span>
                </div>
              ) : (
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <ol className="list-decimal list-inside space-y-1 text-sm text-muted-foreground">
              <li>
                Visit the DeepSeek platform at https://platform.deepseek.com
              </li>
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              The AI Assistant will be available to students on the Study page
              when enabled. Students can click the floating AI button to open a
              chat interface and ask questions about the current study material.
            </p>
          </div>
        </CardContent>
      </Card>
