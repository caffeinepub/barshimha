...
import { Eye, FileText, Image, Save, Settings, Upload } from "lucide-react";
...
export function AdminPaymentConfiguration() {
  ...
  const { data: currentBarcodeUrl } = useFileUrl(
  ...
  const [selectedBarcodeFile, setSelectedBarcodeFile] = useState<File | null>(
    null,
  );
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);

  const handleBarcodeFileSelect = (
...
- Any additional instructions"
            ...
            />
            <p className="text-xs text-muted-foreground">
              This information will be displayed to students when they select
              local bank transfer
            </p>
          </div>
        ...
        <CardContent className="pt-6">
          <div className="space-y-2">
            ...
            <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
              <li>
                • STC Pay barcode should be clear and high-resolution for easy
                scanning
              </li>
              <li>• Include all necessary bank details for local transfers</li>
              <li>
                • Changes will be immediately visible to students on their
                payment page
              </li>
              <li>
                • Test the configuration by viewing the student payment page
              </li>
            </ul>
          </div>
        </CardContent>
