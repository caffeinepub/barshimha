...
import {
  ...
  useGetComments,
  useGetQuestions,
  useGetQuestionsByFilter,
  useGetQuestionsByMode,
  useGetStudyStatistics,
  useRemoveBookmark,
  useSaveStudyStatistics,
  useSearchQuestions,
...
export function Study() {
  ...
  const _canAccessStudy = !isStudent || hasActivePayment;
  ...
  const { data: _filteredByFilter = [] } = useGetQuestionsByFilter(
  ...
  const getFilteredQuestions = useCallback(() => {
    ...
    if (questionLimit !== "all") {
      const limit = Number.parseInt(questionLimit);
  ...
  const startSession = () => {
    ...
    const finalTimeLimit =
      selectedMode === StudyMode.timed
        ? timeLimit === "custom"
          ? Number.parseInt(customTimeLimit)
    ...
    toast.success(
      `Study session started with ${filteredQuestions.length} questions!`,
    );
  };
  ...
  const handleCompleteSession = async () => {
    ...
    try {
      ...
      const accuracy =
        Number(questionsAttempted) > 0
          ? (Number(questionsCorrect) / Number(questionsAttempted)) * 100
          : 0;
      ...
      if (session.mode === StudyMode.practice) {
        updatedStats = {
          ...
          practiceModeStats: {
            questionsAttempted:
              currentStats.practiceModeStats.questionsAttempted +
              questionsAttempted,
            questionsCorrect:
              currentStats.practiceModeStats.questionsCorrect +
              questionsCorrect,
            accuracyPercentage: accuracy,
            timeSpent: currentStats.practiceModeStats.timeSpent + timeSpent,
            sessionsCompleted:
              currentStats.practiceModeStats.sessionsCompleted + BigInt(1),
          },
          aggregateStats: {
            totalQuestionsAttempted:
              currentStats.aggregateStats.totalQuestionsAttempted +
              questionsAttempted,
            totalQuestionsCorrect:
              currentStats.aggregateStats.totalQuestionsCorrect +
              questionsCorrect,
            overallAccuracy:
              Number(
                currentStats.aggregateStats.totalQuestionsAttempted +
                ...
                : 0,
            totalTimeSpent:
              currentStats.aggregateStats.totalTimeSpent + timeSpent,
            totalSessionsCompleted:
              currentStats.aggregateStats.totalSessionsCompleted + BigInt(1),
          },
        };
        ...
        updatedStats = {
          ...
          timedModeStats: {
            questionsAttempted:
              currentStats.timedModeStats.questionsAttempted +
              questionsAttempted,
            questionsCorrect:
              currentStats.timedModeStats.questionsCorrect + questionsCorrect,
            accuracyPercentage: accuracy,
            timeSpent: currentStats.timedModeStats.timeSpent + timeSpent,
            sessionsCompleted:
              currentStats.timedModeStats.sessionsCompleted + BigInt(1),
          },
          aggregateStats: {
            totalQuestionsAttempted:
              currentStats.aggregateStats.totalQuestionsAttempted +
        ...
        updatedStats = {
          ...
          reviewModeStats: {
            questionsAttempted:
              currentStats.reviewModeStats.questionsAttempted +
              questionsAttempted,
            questionsCorrect:
              currentStats.reviewModeStats.questionsCorrect + questionsCorrect,
            accuracyPercentage: accuracy,
            timeSpent: currentStats.reviewModeStats.timeSpent + timeSpent,
            sessionsCompleted:
              currentStats.reviewModeStats.sessionsCompleted + BigInt(1),
          },
          aggregateStats: {
            totalQuestionsAttempted:
              currentStats.aggregateStats.totalQuestionsAttempted +
  ...
  const isConfigurationValid = () => {
    if (selectedMode === StudyMode.timed) {
      const finalTimeLimit =
        timeLimit === "custom" ? customTimeLimit : timeLimit;
      return finalTimeLimit && Number.parseInt(finalTimeLimit) > 0;
    }
    return true;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center space-y-4">
        <h1 className="text-3xl font-bold">Study Center</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Your unified study experience for SMLE exam preparation. Configure
          your session and start studying immediately.
        </p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Choose your preferred study mode for a customized learning
                experience
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[StudyMode.practice, StudyMode.timed, StudyMode.review].map(
                  (mode) => {
          ...
          <div className="flex justify-center">
            <Button
              ...
              disabled={
                !isConfigurationValid() || getFilteredQuestions().length === 0
              }
            >
                ...
                <div className="flex items-center space-x-4">
                  ...
                  {session.mode === StudyMode.timed &&
                    timeRemaining !== null && (
                      <Badge
                        variant={
                          timeRemaining < 300 ? "destructive" : "secondary"
                        }
                      >
              ...
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="space-y-2">
                    ...
                    <CardDescription>{currentQuestion.domain}</CardDescription>
                  </div>
                </div>
              </CardHeader>
                ...
                <div className="space-y-4">
                  {currentQuestion.questionType === QuestionType.numeric ? (
                    ...
                    <RadioGroup
                      ...
                      onValueChange={(value) =>
                        handleAnswerChange(Number.parseInt(value), true)
                      }
                      disabled={showAnswer}
                    >
                      {currentQuestion.options.map((option, index) => (
                        <div
                          key={`single-${option.substring(0, 20)}`}
                    ...
                    <div className="space-y-2">
                      ...
                      {currentQuestion.options.map((option, index) => (
                        <div
                          key={`multi-${option.substring(0, 20)}`}
                          className="flex items-center space-x-2"
                        >
                          <Checkbox
                            id={`option-${index}`}
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {new Date(
                              Number(comment.createdAt) / 1000000,
                            ).toLocaleDateString()}
                          </span>
                        </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  <p className="text-green-700 dark:text-green-300">
                    ...
                    {getModeLabel(session.mode).toLowerCase()} session.
                  </p>
